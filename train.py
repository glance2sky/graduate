...
if not foreground_saved:
    context_frame_num = cp.getint(method, 'context_frame_num')  # 4
    context_of_num = cp.getint(method, 'context_of_num')   # 4
    border_mode = cp.get(method, 'border_mode')   # predict
    if modality == 'raw_datasets':
        file_format = frame_size[dataset_name][2]
    elif modality == 'raw2flow':  # 1
        file_format1 = frame_size[dataset_name][2]   #  (240, 360, '.tif', 1)
        file_format2 = '.npy'
    else:
        file_format = '.npy'
    ...
    if modality == 'raw2flow':
        ...
        # dataset是rgb帧，dataset2是光流帧，dataset2需要自己建立
    else:
    ...
    for iidx in range(dataset.__len__()):
        ...
        batch, _ = dataset.__getitem__(idx)  # rgb帧提取
        if modality == 'raw2flow':
            batch2, _ = dataset2.__getitem__(idx) # 光流帧提取
