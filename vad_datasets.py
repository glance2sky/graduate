...
class ped_dataset(Dataset):
    ...
    def __init__(self, dir, mode='train', context_frame_num=0, border_mode='elastic', file_format='.tif', all_bboxes=None, patch_size=32):
        ...
        self.context_frame_num = context_frame_num # 传入1
    ...
    def dataset_init(self): # 将训练集的所有视频帧的地址合并一起
        if self.mode == 'train':
            data_dir = os.path.join(self.dir, 'Train')  # 数据集UCSDped2目录的子目录就是Train和Test
        elif self.mode == 'test':
        ...
        if self.mode == 'train':
            ...
            for video in sorted(video_dir_list):
                ...
                if 'Train' in video_name:
                    self.videos[video_name] = {}  # videos是个字典，里面每个元素的值全是字典，元素名称是数据集视频名称，如Train001，元素值是另一个字典。每个字典代表一个视频，16个
                    self.videos[video_name]['path'] = video
                    self.videos[video_name]['frame'] = glob.glob(os.path.join(video, '*'+self.file_format)) #也是地址
                    self.videos[video_name]['frame'].sort()
                    self.videos[video_name]['length'] = len(self.videos[video_name]['frame'])
                    self.frame_video_idx += [idx] * self.videos[video_name]['length'] # 为了将所有的视频帧合并在一起并作区分，编号
                    idx += 1

            # merge different frames of different videos into one list 混合所有帧进入一个列表
            for _, cont in self.videos.items():
                self.all_frame_addr += cont['frame']
            self.tot_frame_num = len(self.all_frame_addr)

        elif self.mode == 'test':  # 多了一个gt
            dir_list = glob.glob(os.path.join(data_dir, '*'))
            video_dir_list = [] # 所有视频地址列表
            gt_dir_list = [] # 所有gt地址列表
    ...
    def context_range(self, indice):
        if self.border_mode == 'elastic':  # 传入hard
            # check head and tail
            ...
            # check head and tail 就是从所有帧里选一帧，把这一帧前后的一帧一起挑出来，作为一个数据，这样的话第一帧和最后一帧只能挑后面和前面的帧，这就是分支语句的作用。
            if indice - self.context_frame_num < 0:
                start_idx = 0
            else:
                start_idx = indice - self.context_frame_num
            ...
            if indice + self.context_frame_num > self.tot_frame_num - 1:
                ...
                end_idx = indice + self.context_frame_num
            need_context_num = 2 * self.context_frame_num + 1  # 长度

        center_idx = self.frame_video_idx[indice] # 从所有视频帧集合里的编号来找所属的视频编号
        video_idx = self.frame_video_idx[start_idx:end_idx + 1] # 切片，片长3，片中对应所属视频编号
        pad = need_context_num - len(video_idx) # 0
        if pad > 0:
            if start_idx == 0:
                video_idx = [video_idx[0]] * pad + video_idx
            else:
                video_idx = video_idx + [video_idx[-1]] * pad
        tmp = np.array(video_idx) - center_idx # 检查切片中有没有属于不同视频编号的
        offset = tmp.sum()
        if tmp[0] != 0 and tmp[-1] != 0:  # extreme condition that is not likely to happen # 不可能发生
        ...
        if pad == 0 and offset == 0:  # all frames are from the same video
            idx = [x for x in range(start_idx, end_idx+1)] # idx 是一个长度是三，内容是所有帧集合的idx的列表。
            return idx
        else: # 切片内有不同视频
    ...
    def __getitem__(self, indice):
        ...
        if self.mode == 'train':
            if self.context_frame_num == 0:
                ...
                frame_range = self.context_range(indice=indice) # 输出一个长度是3的列表，里面是3个连续帧的编号
                img_batch = []
